...
{
    public class NotifyPaymentRejectConsumer : INotifyPaymentRejectConsumer
    {
        private readonly IBillQueueManager _billQueueManager;

        public NotifyPaymentRejectConsumer(
        ...
        public async Task ConsumeAsync(IModel channel, ulong deliveryTag, byte[] body)
        {
            ...
            {
                // Parse body
                var data = JsonConvert.DeserializeObject<NotifyPaymentReject>(Encoding.UTF8.GetString(body));
