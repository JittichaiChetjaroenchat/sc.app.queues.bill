...
{
    public class NotifyBillSummaryConsumer : INotifyBillSummaryConsumer
    {
        private readonly IBillQueueManager _billQueueManager;

        public NotifyBillSummaryConsumer(
        ...
        public async Task ConsumeAsync(IModel channel, ulong deliveryTag, byte[] body)
        {
            ...
            {
                // Parse body
                var data = JsonConvert.DeserializeObject<NotifyBillSummary>(Encoding.UTF8.GetString(body));
