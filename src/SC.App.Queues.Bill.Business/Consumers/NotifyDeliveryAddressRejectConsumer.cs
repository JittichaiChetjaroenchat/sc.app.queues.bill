...
{
    public class NotifyDeliveryAddressRejectConsumer : INotifyDeliveryAddressRejectConsumer
    {
        private readonly IBillQueueManager _billQueueManager;

        public NotifyDeliveryAddressRejectConsumer(
        ...
        public async Task ConsumeAsync(IModel channel, ulong deliveryTag, byte[] body)
        {
            ...
            {
                // Parse body
                var data = JsonConvert.DeserializeObject<NotifyDeliveryAddressReject>(Encoding.UTF8.GetString(body));
