...
{
    public class NotifyBillConfirmConsumer : INotifyBillConfirmConsumer
    {
        private readonly IBillQueueManager _billQueueManager;

        public NotifyBillConfirmConsumer(
        ...
        public async Task ConsumeAsync(IModel channel, ulong deliveryTag, byte[] body)
        {
            ...
            {
                // Parse body
                var data = JsonConvert.DeserializeObject<NotifyBillConfirm>(Encoding.UTF8.GetString(body));
