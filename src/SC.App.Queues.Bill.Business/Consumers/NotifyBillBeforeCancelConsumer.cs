...
{
    public class NotifyBillBeforeCancelConsumer : INotifyBillBeforeCancelConsumer
    {
        private readonly IBillQueueManager _billQueueManager;

        public NotifyBillBeforeCancelConsumer(
        ...
        public async Task ConsumeAsync(IModel channel, ulong deliveryTag, byte[] body)
        {
            ...
            {
                // Parse body
                var data = JsonConvert.DeserializeObject<NotifyBillBeforeCancel>(Encoding.UTF8.GetString(body));
