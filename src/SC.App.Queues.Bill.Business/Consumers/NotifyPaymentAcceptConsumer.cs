...
{
    public class NotifyPaymentAcceptConsumer : INotifyPaymentAcceptConsumer
    {
        private readonly IBillQueueManager _billQueueManager;

        public NotifyPaymentAcceptConsumer(
        ...
        public async Task ConsumeAsync(IModel channel, ulong deliveryTag, byte[] body)
        {
            ...
            {
                // Parse body
                var data = JsonConvert.DeserializeObject<NotifyPaymentAccept>(Encoding.UTF8.GetString(body));
