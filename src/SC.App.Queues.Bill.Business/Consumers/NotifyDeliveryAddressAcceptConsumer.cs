...
{
    public class NotifyDeliveryAddressAcceptConsumer : INotifyDeliveryAddressAcceptConsumer
    {
        private readonly IBillQueueManager _billQueueManager;

        public NotifyDeliveryAddressAcceptConsumer(
        ...
        public async Task ConsumeAsync(IModel channel, ulong deliveryTag, byte[] body)
        {
            ...
            {
                // Parse body
                var data = JsonConvert.DeserializeObject<NotifyDeliveryAddressAccept>(Encoding.UTF8.GetString(body));
