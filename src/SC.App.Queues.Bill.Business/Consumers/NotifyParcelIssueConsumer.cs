...
{
    public class NotifyParcelIssueConsumer : INotifyParcelIssueConsumer
    {
        private readonly IBillQueueManager _billQueueManager;

        public NotifyParcelIssueConsumer(
        ...
        public async Task ConsumeAsync(IModel channel, ulong deliveryTag, byte[] body)
        {
            ...
            {
                // Parse body
                var data = JsonConvert.DeserializeObject<NotifyParcelIssue>(Encoding.UTF8.GetString(body));
