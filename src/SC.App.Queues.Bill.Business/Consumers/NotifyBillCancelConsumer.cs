...
{
    public class NotifyBillCancelConsumer : INotifyBillCancelConsumer
    {
        private readonly IBillQueueManager _billQueueManager;

        public NotifyBillCancelConsumer(
        ...
        public async Task ConsumeAsync(IModel channel, ulong deliveryTag, byte[] body)
        {
            ...
            {
                // Parse body
                var data = JsonConvert.DeserializeObject<NotifyBillCancel>(Encoding.UTF8.GetString(body));
