...
namespace SC.App.Queues.Bill.Business.Helpers
{
    ...
    {
        public static decimal CalculateCost(List<Order.Database.Models.Order> orders, BillShipping billShipping)
        {
        ...
        private static decimal CalculateCostRange(List<Order.Database.Models.Order> orders, BillShipping billShipping)
        {
        ...
        private static decimal CalculateCostTotal(List<Order.Database.Models.Order> orders, BillShipping billShipping)
        {
            var amount = OrderHelper.GetAmount(orders);
            var price = OrderHelper.GetPrice(orders);
            var isHitFreeRule = IsHitFreeRule(amount, price, billShipping);
        ...
        private static EnumShippingCostType GetCostType(BillShipping billShipping)
        {
